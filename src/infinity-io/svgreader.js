    ...
    function build(opts) {
        ...
        svg.Transform = function (v) {
            ...
            this.Type.translate = function (s) {
                ...
                this.apply = function (ctx) {
                    ctx.transform = ctx.transform.translated(this.p.x || 0.0, this.p.y || 0.0);
                }
                this.unapply = function (ctx) {
                    ctx.transform = ctx.transform.translated(-1.0 * this.p.x || 0.0, -1.0 * this.p.y || 0.0);
                }
            ...
            this.Type.rotate = function (s) {
                ...
                this.cy = a[2] || 0;
                this.apply = function (ctx) {
                    ctx.transform = ctx.transform.translated(this.cx, this.cy);
            ...
            this.Type.scale = function (s) {
                this.p = svg.CreatePoint(s);
                this.apply = function (ctx) {
                    ctx.transform = ctx.transform.scaled(this.p.x || 1.0, this.p.y || this.p.x || 1.0);
                }
                this.unapply = function (ctx) {
                    ctx.transform = ctx.transform.scaled(1.0 / this.p.x || 1.0, 1.0 / this.p.y || this.p.x || 1.0);
                }
                this.applyToPoint = function (p) {
        ...
        svg.AspectRatio = function (ctx, aspectRatio, width, desiredWidth, height, desiredHeight, minX, minY, refX, refY) {
            ...
            if (refX.hasValue() && refY.hasValue()) {
                ctx.transform = ctx.transform.translated(-scaleMin * refX.toPixels('x'), -scaleMin * refY.toPixels('y'));
            }
            else {
                // align
                if (align.match(/^xMid/) && ((meetOrSlice == 'meet' && scaleMin == scaleY) || (meetOrSlice == 'slice' && scaleMax == scaleY)))
                    ctx.transform = ctx.transform.translated(width / 2.0 - desiredWidth / 2.0, 0);
                if (align.match(/YMid$/) && ((meetOrSlice == 'meet' && scaleMin == scaleX) || (meetOrSlice == 'slice' && scaleMax == scaleX)))
                    ctx.transform = ctx.transform.translated(0, height / 2.0 - desiredHeight / 2.0);
                if (align.match(/^xMax/) && ((meetOrSlice == 'meet' && scaleMin == scaleY) || (meetOrSlice == 'slice' && scaleMax == scaleY)))
                    ctx.transform = ctx.transform.translated(width - desiredWidth, 0);
                if (align.match(/YMax$/) && ((meetOrSlice == 'meet' && scaleMin == scaleX) || (meetOrSlice == 'slice' && scaleMax == scaleX)))
                    ctx.transform = ctx.transform.translated(0, height - desiredHeight);
            }

            // scale
            if (align == 'none') ctx.transform = ctx.transform.scaled(scaleX, scaleY);
            else if (meetOrSlice == 'meet') ctx.transform = ctx.transform.scaled(scaleMin, scaleMin);
            else if (meetOrSlice == 'slice') ctx.transform = ctx.transform.scaled(scaleMax, scaleMax);

            // translate
            ctx.transform = ctx.transform.translated(minX == null ? 0 : -minX, minY == null ? 0 : -minY);
        }
        ...
        svg.Element.ElementBase = function (node) {
            ...
            this.toScene = function (stack) {
                ...
                if (node) {
                    ...
                    this.setSceneContext(context, node);
                    for (var i = 0; i < this.children.length; i++) {
                        var childNode = this.children[i].toScene(stack);
                        if (childNode) {
                            node.appendChild(childNode);
                        }
                    }
                    this.clearSceneContext(context, node);
            ...
            this.clearSceneContext = function (ctx) {
                // OVERRIDE ME!
            }
                ...
                if (node && node.hasMixin(GStylable)) {
                    ...
                    if (propertySets.indexOf(GStylable.PropertySet.Fill) >= 0) {
                        if (this.style('fill').isUrlDefinition()) {
                            ...
                            if (fillStyle.value == 'none') {
                                propertiesToSet['_fpt'] = null;
                            } else {
                        ...
                        if (this.style('fill-opacity').hasValue()) {
                            var mul = propertiesToSet.hasOwnProperty('_fop') ? propertiesToSet['_fop'] : 1;
                            propertiesToSet['_fop'] = this.style('fill-opacity').numValue() * mul;
                        }
                    }
                    ...
                    if (propertySets.indexOf(GStylable.PropertySet.Border) >= 0) {
                        ...
                        if (this.style('stroke-opacity').hasValue()) {
                            var mul = propertiesToSet.hasOwnProperty('_bop') ? propertiesToSet['_bop'] : 1;
                            propertiesToSet['_bop'] = this.style('stroke-opacity').numValue() * mul;
                        }

                        if (this.style('stroke-width').hasValue()) {
                            propertiesToSet['_bw'] = this.style('stroke-width').toPixels();
                        } else {
                            // default
                        ...
                        if (this.style('stroke-miterlimit').hasValue()) {
                            propertiesToSet['_bml'] = this.style('stroke-miterlimit').numValue();
                        } else {
                            // default
                            propertiesToSet['_bml'] = 4;
                        }
                    ...
                    var properties = [];
                    var values = [];
                    for (var property in propertiesToSet) {
                        properties.push(property);
                        values.push(propertiesToSet[property]);
                    }
        ...
        svg.Element.svg = function (node) {
            ...
            this.clearSceneContext = function (stack) {
                this.baseClearSceneContext(stack);
        ...
        svg.Element.rect.prototype = new svg.Element.RenderedElementBase;
        ...
        svg.Element.circle.prototype = new svg.Element.RenderedElementBase;
        ...
        svg.Element.ellipse.prototype = new svg.Element.RenderedElementBase;
        ...
        svg.Element.pattern = function (node) {
            ...
            this.createPattern = function (ctx, element) {
                ...
                if (this.attribute('x').hasValue() && this.attribute('y').hasValue()) {
                    cctx.transform = ctx.transform.translated(this.attribute('x').toPixels('x', true), this.attribute('y').toPixels('y', true));
                }
                // render 3x3 grid so when we transform there's no white space on edges
                for (var x = -1; x <= 1; x++) {
                    for (var y = -1; y <= 1; y++) {
                        cctx.save();
                        cctx.transform = ctx.transform.translated(x * c.width, y * c.height);
                        tempSvg.render(cctx);
                        cctx.restore();
                    }
                }
        ...
        svg.Element.marker = function (node) {
            ...
            this.render = function (ctx, point, angle) {
                ctx.transform = ctx.transform.translated(point.x, point.y);
                if (this.attribute('orient').valueOrDefault('auto') == 'auto') ctx.rotate(angle);
                if (this.attribute('markerUnits').valueOrDefault('strokeWidth') == 'strokeWidth') ctx.transform = ctx.transform.scaled(ctx.lineWidth, ctx.lineWidth);
                ctx.save();
                ...
                ctx.restore();
                if (this.attribute('markerUnits').valueOrDefault('strokeWidth') == 'strokeWidth') ctx.transform = ctx.transform.scaled(1 / ctx.lineWidth, 1 / ctx.lineWidth);
                if (this.attribute('orient').valueOrDefault('auto') == 'auto') ctx.rotate(-angle);
                ctx.transform = ctx.transform.translated(-point.x, -point.y);
            }
        }
        ...
        svg.Element.image = function (node) {
            ...
            this.renderChildren = function (ctx) {
                ...
                ctx.transform = ctx.transform.translated(x, y);
                svg.AspectRatio(ctx,
                    this.attribute('preserveAspectRatio').value,
                    width,
                    this.img.width,
        ...
        svg.Element.use = function (node) {
            ...
            this.setContext = function (ctx) {
                this.baseSetContext(ctx);
                if (this.attribute('x').hasValue()) ctx.transform = ctx.transform.translated(this.attribute('x').toPixels('x'), 0);
                if (this.attribute('y').hasValue()) ctx.transform = ctx.transform.translated(0, this.attribute('y').toPixels('y'));
            }
